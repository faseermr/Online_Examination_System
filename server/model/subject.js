...
module.exports = {
  getSubjectList: (result) => {
    dbConn.query(
      `SELECT * FROM subject INNER JOIN class on class.clsid = subject.clsid`,
      (err, res) => {
        if (err) {
          result(err, res);
        } else {
  ...
  updateSubjectStatus: (sub_status, subid, result) => {
    dbConn.query(
      `Update subject set status =? where subid=?`,
      [sub_status, subid],
      (err, res) => {
        if (err) {
          result(err, res);
  ...
  create: (data, result) => {
    ...
    dbConn.query(
      `Insert into subject (name,clsid) values (?,?)`,
      [data.subject, data.clsid],
      (err, res) => {
        if (err) {
          result(err, res);
        } else {
  ...
  update: (id, data, result) => {
    dbConn.query(
      `Update subject set name =?, clsid =? where subid=?`,
      [data.subject, data.clsid, id],
      (err, res) => {
        if (err) {
          result(err, res);
        } else {
