...
const answerQuestion = async(questionid) => {
    if(answer){
        ...
        await examServices.answerQuestion({
            qid:questionid,
            stuid:userId,
            ans:answer
        }).then(res =>{
...
const getQuestionBySubjectAndStudent = async (subject) => {
    setRefresh(true)
    if(user.length > 0 ) {
        const res =await examServices.getQuestionBySubjectAndStudent(subject,user[0].stuid)
...
const submitAnswer = async (subject) => {
    ...
    if(option){
        await examServices.submitAnswer({
            student:userId,
            subject:subject
        }).then(res => {
                ...
                return(
                    <div className='mcard' key={idx}>
                        <div>
                            ...
                            <span>
                                <input type="radio"  name={idx} value={val.ans1} defaultChecked={val.ans1 == val.ans} onChange={()=>setAnswer(val.ans1)}/>
                                <label className='labelMargin'>{val.ans1}</label>
                                <br/>
                                <input type="radio" name={idx} value={val.ans2} defaultChecked={val.ans2 == val.ans} onChange={()=>setAnswer(val.ans2)}/>
                                <label className='labelMargin'>{val.ans2}</label>
                                <br/>
                                <input type="radio"  name={idx} value={val.ans3} defaultChecked={val.ans3 == val.ans} onChange={()=>setAnswer(val.ans3)}/>
                                <label className='labelMargin'>{val.ans3}</label>
                                <br/>
                                <input type="radio"  name={idx} value={val.ans4} defaultChecked={val.ans4 == val.ans} onChange={()=>setAnswer(val.ans4)}/>
                        ...
                        <button className='btn btn-dark' 
                                ...
                                disabled={lastIndex == question.length + 1 || admin.length >0 && lastIndex == question.length }
                                >Next</button> 
